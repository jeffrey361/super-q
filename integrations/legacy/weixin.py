...
def _response_error_summary(response: Optional[Dict[str, Any]]) -> str:
    if not isinstance(response, dict):
        return str(response)
    ret = response.get("ret")
    errcode = response.get("errcode")
    errmsg = response.get("errmsg") or response.get("err_msg") or response.get("message") or ""
    extras = []
    if ret not in (None, ""):
        extras.append(f"ret={ret}")
    if errcode not in (None, ""):
        extras.append(f"errcode={errcode}")
    if errmsg:
        extras.append(f"errmsg={errmsg}")
    return ", ".join(extras) if extras else json.dumps(response, ensure_ascii=False)[:200]


def _ensure_api_success(endpoint: str, response: Dict[str, Any]) -> Dict[str, Any]:
    ret = response.get("ret", 0)
    errcode = response.get("errcode", 0)
    if ret not in (0, None) or errcode not in (0, None):
        raise RuntimeError(f"iLink POST {endpoint} business error: {_response_error_summary(response)}")
    return response
...
def _audit_send_result(
    *,
    ...
    meta = metadata or {}
    logger.info(
        "[%s] outbound_send_audit success=%s chat_id=%s context_token=%s client_id=%s job_id=%s session_id=%s delivery_target=%s response=%s error=%s",
        adapter_name,
        success,
        _safe_id(chat_id),
        _safe_id(context_token),
        client_id,
        meta.get("job_id") or "-",
        meta.get("session_id") or "-",
        meta.get("deliver") or meta.get("target") or "-",
...
async def _api_post(
    ...
    *,
    ...
    async with session.post(url, data=body, headers=_headers(token, body), timeout=timeout) as response:
        ...
        return _ensure_api_success(endpoint, parsed)
