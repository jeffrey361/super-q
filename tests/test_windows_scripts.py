...
def test_start_superq_script_launches_goldminer_and_daemon() -> None:
    script = (SCRIPTS / "start_superq.ps1").read_text(encoding="utf-8")
...
def test_stop_superq_script_stops_daemon_and_goldminer() -> None:
    script = (SCRIPTS / "stop_superq.ps1").read_text(encoding="utf-8")
...
def test_double_click_bat_wrappers_call_powershell_scripts() -> None:
    start_bat = (SCRIPTS / "start_superq.bat").read_text(encoding="utf-8")
    stop_bat = (SCRIPTS / "stop_superq.bat").read_text(encoding="utf-8")
