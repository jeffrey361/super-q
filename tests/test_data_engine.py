...
# Feature: superQ-v2, Property 4: (symbol, date) 唯一约束防止重复写入
@given(
...
# Feature: superQ-v2, Property 5: 增量同步的 start_date 由 last_date 决定
@given(
    symbol=st.text(min_size=6, max_size=6, alphabet="0123456789"),
...
# Feature: superQ-v2, Property 6: 空增量数据不触发写入
...
def test_sync_all_processes_each_symbol_once(monkeypatch) -> None:
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp_dir:
        ...
        def fake_sync(symbol):  # type: ignore[no-untyped-def]
            calls.append(symbol)
            return SyncResult(symbol=symbol, status="skip")

        monkeypatch.setattr(engine, "sync_symbol", fake_sync)
        summary = engine.sync_all(["000001", "000002", "000001", "600000"])
...
# Feature: superQ-v2, Property 8: akshare 调用始终使用前复权参数
@given(symbol=st.text(min_size=6, max_size=6, alphabet="0123456789"))
@h_settings(max_examples=50, deadline=None)
