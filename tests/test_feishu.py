...
# Feature: superQ-v2, Property 10: 飞书通知包含所有选股结果
@given(
    symbols=st.lists(
...
def test_notification_contains_all_symbols(symbols: list[str]) -> None:
    ...
    with (
        patch.object(FeishuNotifier, "_get_xueqiu_mapping", return_value={}),
...
# Feature: superQ-v2, Property 11: 飞书通知使用 ConfigManager 中的 Webhook URL
@given(
...
# Feature: superQ-v2, Property 12: HTTP 失败时记录 ERROR 日志
...
def test_feishu_card_includes_account_summary() -> None:
    """飞书卡片应可附带掘金账户摘要。"""
    notifier = FeishuNotifier(make_settings())

    with patch.object(FeishuNotifier, "_get_xueqiu_mapping", return_value={}):
    ...
    assert "账户余额：88000.50" in json.dumps(card, ensure_ascii=False)
    assert "300054 综合分 80" in json.dumps(card, ensure_ascii=False)
